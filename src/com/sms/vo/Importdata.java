package com.sms.vo;

import java.sql.Date;

public class Importdata
{
	int exam_number;//准考证号
	String student_name;//姓名
	String name_old;//曾用名
	String sex;//性别
	String info_department;//院系
	String student_major;//专业
	int grade;//级
	String edu_systme;//学制
	Date birth;//出生日期
	String info_political;//政治面貌
	int id_number;//身份证号
	String nation;//民族
	String origo;//籍贯
	String origin;//生源地
	String school;//来校前所在学校
	String address;//家庭住址
	int zip;//邮政编码
	int info_tel;//联系电话
	int is_fresh;//是否应届生
	String student_status;//学籍状态
	public int getExam_number()
	{
		return exam_number;
	}
	public void setExam_number(int exam_number)
	{
		this.exam_number = exam_number;
	}
	public String getStudent_name()
	{
		return student_name;
	}
	public void setStudent_name(String student_name)
	{
		this.student_name = student_name;
	}
	public String getName_old()
	{
		return name_old;
	}
	public void setName_old(String name_old)
	{
		this.name_old = name_old;
	}
	public String getSex()
	{
		return sex;
	}
	public void setSex(String sex)
	{
		this.sex = sex;
	}
	public String getInfo_department()
	{
		return info_department;
	}
	public void setInfo_department(String info_department)
	{
		this.info_department = info_department;
	}
	public String getStudent_major()
	{
		return student_major;
	}
	public void setStudent_major(String student_major)
	{
		this.student_major = student_major;
	}
	public int getGrade()
	{
		return grade;
	}
	public void setGrade(int grade)
	{
		this.grade = grade;
	}
	public String getEdu_systme()
	{
		return edu_systme;
	}
	public void setEdu_systme(String edu_systme)
	{
		this.edu_systme = edu_systme;
	}
	public Date getBirth()
	{
		return birth;
	}
	public void setBirth( Date date)
	{
		this.birth = date;
	}
	public String getInfo_political()
	{
		return info_political;
	}
	public void setInfo_political(String info_political)
	{
		this.info_political = info_political;
	}
	public int getId_number()
	{
		return id_number;
	}
	public void setId_number(int id_number)
	{
		this.id_number = id_number;
	}
	public String getNation()
	{
		return nation;
	}
	public void setNation(String nation)
	{
		this.nation = nation;
	}
	public String getOrigo()
	{
		return origo;
	}
	public void setOrigo(String origo)
	{
		this.origo = origo;
	}
	public String getOrigin()
	{
		return origin;
	}
	public void setOrigin(String origin)
	{
		this.origin = origin;
	}
	public String getSchool()
	{
		return school;
	}
	public void setSchool(String school)
	{
		this.school = school;
	}
	public String getAddress()
	{
		return address;
	}
	public void setAddress(String address)
	{
		this.address = address;
	}
	public int getZip()
	{
		return zip;
	}
	public void setZip(int zip)
	{
		this.zip = zip;
	}
	public int getInfo_tel()
	{
		return info_tel;
	}
	public void setInfo_tel(int info_tel)
	{
		this.info_tel = info_tel;
	}
	public int getIs_fresh()
	{
		return is_fresh;
	}
	public void setIs_fresh(int is_fresh)
	{
		this.is_fresh = is_fresh;
	}
	public String getStudent_status()
	{
		return student_status;
	}
	public void setStudent_status(String student_status)
	{
		this.student_status = student_status;
	}
	
	
}
